...
#python Extract_data_cellFeatures_spotFeatures.py "all_data_file.csv"
...
channels = ["../input/*C0.xls", "../input/*C1.xls","../input/*C2.xls"]
chanId = ["0","1","2"]
n = 0
        ...
        while row_id < byroi_sheet.nrows:
            ...
            if row[0] == u"ROI Id":
                ...
                while row[0] != u"--------":
                    ...
                    values = [chanId[n], day, pos, extracted_roi_id, extracted_roi_area,
                              float(row[1 - 1]), float(row[3 - 1]), float(row[4 - 1]), float(row[6 - 1]),
                              float(row[maxInt[n] - 1]), float(row[meanInt[n] - 1])]
                    output.append([values])
                    row_id += 1
                    if not row_id < byroi_sheet.nrows:
                        break
